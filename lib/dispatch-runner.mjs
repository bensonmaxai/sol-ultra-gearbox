...
function extractDeliverable(summary, marker) {
  for (const text of [...(summary?.finalTexts ?? [])].reverse()) {
    ...
    const deliverable = text.replace(marker, "").trim();
...
export async function runIsolatedRole({
  ...
  try {
    ...
    const preliminary = summary
      ? verifyIsolatedRoot({
          ...
          before: { changedFiles: [] },
          after: { changedFiles },
          cleanup: { passed: true, commandExitedZero: execution.code === 0, timedOut: execution.timedOut },
        })
      : null;
    const deliverable = preliminary?.pass ? extractDeliverable(summary, marker) : null;
    if (deliverable) {
      try {
  ...
  result.checks.deliverableValid = result.checks.deliverableValid === true && markerReturned && deliveryAccepted;
