...
import { readyStageIds, scheduleWorkflow, selectCandidateBatch } from "./workflow-scheduler.mjs";
...
function projectedWithReadiness(plan, state) {
  ...
  for (const event of readinessEvents) projected.stages[event.stageId].state = "ready";
...
function actionFor({ plan, planHash, batchId: id, stageId, canaryStageId, deferredStageIds, packet, decision }) {
  if (decision.effectiveShape === "root_inline") {
    return { kind: "root_inline", stageId, packet, decision };
  }
  return {
    kind: "materialize",
...
export function planNextWorkflowAction({ plan, planHash, state, policy, capabilities, roleSpecs = ROLE_SPECS }) {
  if (state.activeBatch) {
    ...
    return {
      readinessEvents: [],
      batchEvent: null,
      action: actionFor({ plan, planHash, batchId: id, stageId, canaryStageId: state.activeBatch.canaryStageId, deferredStageIds: candidate.stageIds.filter((id) => id !== stageId && id !== state.activeBatch.canaryStageId), packet: packets.get(stageId), decision }),
    };
  }
...
export function providerForDecision(decision) {
  ...
  return {
    capabilities: ["materialize", "readiness", "collectEvidence", "close"],
    materialize,
    readiness(input) {
      inputFor(decision, input);
    ...
    collectEvidence(input) {
      inputFor(decision, input);
      return { kind: "collect_evidence", taskHash: decision.taskHash, executionShape: decision.effectiveShape };
    },
    close(input) {
      inputFor(decision, input);
      return { kind: "provider_close", taskHash: decision.taskHash, executionShape: decision.effectiveShape };
    },
  };
}
