...
export const ACCEPTANCE_SCENARIOS = Object.freeze([
  ...
  { id: "Q3_ISOLATED_TERRA_NO_NATIVE_SCHEMA", selectedShape: "isolated_role_root", reasonCode: "DELEGATE_ISOLATED_SCHEMA_UNAVAILABLE", executor: "isolated" },
...
const SCENARIO_PACKET_OVERRIDES = Object.freeze({
  ...
  Q3_ISOLATED_TERRA_NO_NATIVE_SCHEMA: { responsibility: "exploration" },
  ...
  Q5_ROOT_HIGH_RISK: {
    ...
    writeScope: ["fixture.txt"],
    batch: { writerCount: 1 },
    riskSignals: { highRisk: true },
  },
...
const SCENARIO_CAPABILITY_OVERRIDES = Object.freeze({
  Q3_ISOLATED_TERRA_NO_NATIVE_SCHEMA: Object.freeze({ agentTypeVisible: false }),
});
...
export function planAcceptanceScenario({ scenario, policy, capabilities, roleSpecs, plan = planDispatch }) {
  const scenarioCapabilities = {
    ...capabilities,
    ...(SCENARIO_CAPABILITY_OVERRIDES[scenario?.id] ?? {}),
  };
  const decision = plan({ policy, packet: createAcceptancePacket(scenario), capabilities: scenarioCapabilities, roleSpecs });
...
export const PARALLEL_QUESTION_KEYS = Object.freeze([...QUESTION_KEYS, "workflowCanary"]);
...
export function validateAcceptanceDeliverable(id, source) {
  ...
  if (id === "Q3_ISOLATED_TERRA_NO_NATIVE_SCHEMA") {
...
function parallelPasses(topology) {
  ...
  const exactChildren = Array.isArray(children) && children.length === ACCEPTANCE_PARALLEL_CHILDREN.length &&
    ACCEPTANCE_PARALLEL_CHILDREN.every((expected) => children.some((child) =>
      ...
      child?.declaredReadScope === expected.readScope &&
      child?.markerReturned === true,
    ));
  const workflowCanary = topology?.workflowCanary;
  const canaryPasses = workflowCanary?.firstRole === "luna_clerk" &&
    workflowCanary?.firstChildPersisted === true &&
    workflowCanary?.listObservedBetweenSpawns === true &&
    workflowCanary?.listReceiptRunningOrCompleted === true &&
    workflowCanary?.secondRole === "terra_explorer" &&
    workflowCanary?.secondSpawnAfterCanary === true;
  return topology?.parent?.model === "gpt-5.6-sol" &&
    ...
    children.every((child) =>
      ...
      typeof child.declaredReadScope === "string" && child.declaredReadScope.length > 0 &&
      tokenUsage(child.tokenUsage),
    ) &&
    children[0].role !== children[1].role &&
    children[0].declaredReadScope !== children[1].declaredReadScope &&
    topology?.writerCount === 0 && topology?.descendantCount === 0 &&
    topology?.spawnsExact === true && topology?.messagesDistinct === true && topology?.lineageExact === true &&
...
function publicQuestion(scenario, result, pass) {
  ...
  if (scenario.parallel) question.workflowCanary = result?.topology?.workflowCanary?.firstRole === "luna_clerk" &&
    result?.topology?.workflowCanary?.firstChildPersisted === true &&
    result?.topology?.workflowCanary?.listObservedBetweenSpawns === true &&
    result?.topology?.workflowCanary?.listReceiptRunningOrCompleted === true &&
    result?.topology?.workflowCanary?.secondRole === "terra_explorer" &&
    result?.topology?.workflowCanary?.secondSpawnAfterCanary === true;
  return question;
}
...
export function validateAcceptanceEvidence(report) {
  ...
  const exactQuestions = questions.length === QUESTION_IDS.length && questions.every((question, index) =>
    exactKeys(question, ACCEPTANCE_SCENARIOS[index].negative ? NEGATIVE_QUESTION_KEYS : ACCEPTANCE_SCENARIOS[index].parallel ? PARALLEL_QUESTION_KEYS : QUESTION_KEYS) &&
    ...
    (!ACCEPTANCE_SCENARIOS[index].parallel || question?.workflowCanary === true),
  );
  const checks = {
    ...
    configBinding: report?.runtimeBinding?.configSha256 === report?.globalConfigBeforeSha256,
