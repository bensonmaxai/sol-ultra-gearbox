...
export const APP_SERVER_ROOT_PROVIDER_CAPABILITIES = Object.freeze([
  "ownerAuthorized",
  "hostLifecycleAvailable",
  "initializeAvailable",
  ...
  "threadReadAvailable",
  "archiveAvailable",
  "persistedRuntimeEvidenceAvailable",
  "writeScopeVerifiable",
  "closeLifecycleVerifiable",
]);
...
export function evaluateAppServerRootProvider(capabilities = null, {
  policy = null,
} = {}) {
  const checks = Object.fromEntries(
    APP_SERVER_ROOT_PROVIDER_CAPABILITIES.map(
      (key) => [key, capabilities?.[key] === true],
    ),
  );
  checks.policyEnabled = (
    policy?.schemaVersion === 2 &&
    policy?.mode === "active" &&
    policy?.rootProvider?.kind === "app_server_root" &&
    policy?.rootProvider?.enabled === true &&
    policy?.rootProvider?.transport === "stdio" &&
    policy?.rootProvider?.protocolVersion === 1
  );
  checks.paidAcceptanceCurrent =
    typeof policy?.rootProvider?.acceptanceBindingSha256 === "string" &&
    HASH.test(policy.rootProvider.acceptanceBindingSha256);
  let reasonCode = "APP_SERVER_ROOT_PROVIDER_READY";
  if (!checks.policyEnabled) reasonCode = "APP_SERVER_ROOT_POLICY_DISABLED";
  else if (!checks.ownerAuthorized) reasonCode = "APP_SERVER_ROOT_OWNER_AUTHORITY_REQUIRED";
  else if (!checks.paidAcceptanceCurrent) reasonCode = "APP_SERVER_ROOT_ACCEPTANCE_UNBOUND";
  else if (!checks.hostLifecycleAvailable || !checks.initializeAvailable ||
    !checks.threadStartAvailable || !checks.threadReadAvailable ||
    !checks.archiveAvailable) reasonCode = "APP_SERVER_ROOT_HOST_UNAVAILABLE";
  else if (!checks.turnStartModelSelection) {
    reasonCode = "APP_SERVER_ROOT_TURN_START_ROUTING_UNAVAILABLE";
  } else if (!checks.persistedRuntimeEvidenceAvailable) {
    reasonCode = "APP_SERVER_ROOT_RUNTIME_EVIDENCE_UNAVAILABLE";
  } else if (!checks.writeScopeVerifiable) {
    reasonCode = "APP_SERVER_ROOT_SCOPE_EVIDENCE_UNAVAILABLE";
  } else if (!checks.closeLifecycleVerifiable) {
    reasonCode = "APP_SERVER_ROOT_LIFECYCLE_UNVERIFIED";
  }
...
export function planRootLaunch({ policy, packet, capabilities, roleSpecs }) {
  ...
  const assessment = evaluateAppServerRootProvider(capabilities, { policy });
  const executable = workflowPolicy.pass === true && assessment.pass === true;
  return {
    schemaVersion: 1,
    taskHash,
    policyMode: policy?.mode ?? "off",
    responsibility: packet.responsibility,
    selectedShape: executable ? "app_server_root" : "root_inline",
    effectiveShape: executable ? "app_server_root" : "root_inline",
    role: null,
    reasonCode: workflowPolicy.pass === false
      ? workflowPolicy.reasonCode
      : executable
        ? routing.root.reasonCode
        : "ROOT_APP_SERVER_PROVIDER_UNAVAILABLE",
    workflowPolicy,
    spawnArgs: null,
    requiresRuntimeEvidence: executable,
    routing,
    provider: {
      requested: "app_server_root",
      selected: executable ? "app_server_root" : "root_inline",
      requestedExecutable: executable,
      executable: true,
      fallbackApplied: !executable,
      delegatedExecution: false,
      reasonCode: workflowPolicy.pass === false
        ? "APP_SERVER_ROOT_WORKFLOW_POLICY_REJECTED"
