...
export function createWorkflowRecord({ previousRecordHash, state, event }) {
  ...
  const approval = event?.type === "approval_recorded";
  ...
  const stageId = initialization || approval ? null : eventData.stageId;
...
export function validateWorkflowRecord(record) {
  ...
  const approval = record.eventType === "approval_recorded";
  if (initialization) {
...
export function replayWorkflowRecords({ plan, records }) {
  if (!plan || !Array.isArray(records) || records.length === 0) {
    throw new TypeError("workflow replay requires a plan and records");
  }
  for (const record of records) assertValidRecord(record);
  if (records[0].eventType !== INITIALIZED) {
    throw new TypeError("workflow replay must begin with initialization");
  }
  ...
  for (const [index, record] of records.entries()) {
    if (record.previousRecordHash !== previousRecordHash) {
      throw new TypeError("workflow record hash chain is broken");
    }
    if (index > 0 && record.eventType === INITIALIZED) {
      throw new TypeError("workflow initialization is duplicated");
    }
    for (const field of ["workflowId", "planHash", "policyMode", "policyHash", "permissionHash", "workspaceHash"]) {
      if (record[field] !== binding[field]) throw new TypeError(`workflow ${field} binding drifted`);
    }
    if (index > 0) {
