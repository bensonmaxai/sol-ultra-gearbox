...
- Before supported actual delegation, run \`gearbox-dispatch plan\` with the packet plus current schema and parent-permission facts. Unknown skills, missing typed capability flags, generic roles, direct core calls, and missing trusted runtime evidence fail closed to \`root_inline\`.
- The only execution shapes are \`root_inline\`, \`typed_child\`, \`isolated_role_root\`, and \`typed_child_bridge\`. Direct Luna/Terra read-only permission mismatches use \`isolated_role_root\`: an isolated root, never a child. It is started only through \`gearbox-dispatch run-isolated\`, never by pretending it has native-child lineage.
...
export async function installDispatchRuntime({
  ...
  if (policy === null || policy.mode !== dispatchMode || policy.allowTypedBridge !== false) {
    throw new Error("active dispatch requires a signed activation policy");
  }
...
const OWNED_PROBE_DIRECTORY =
  /^sol-ultra-gearbox-v2-(?:(?:home-)?(?:luna_clerk|terra_explorer|terra_worker|sol_reviewer|terra_ultra_specialist|terra_max_worker)|sdd|dispatch-(?:luna_clerk|terra_explorer)|dispatch-home-(?:luna_clerk|terra_explorer)|scope-dispatch|packet-dispatch)-[A-Za-z0-9]+$/;
