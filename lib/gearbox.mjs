...
- A legacy activation without a scoped snapshot binds root model and effort to persisted activation smoke and applies the strict safe contract to other semantics; a future apply captures exact scoped semantic equality.
- The App Server／App-thread provider contract is capability-driven and remains non-executable until a policy-enabled host proves owner authority, turn-start model／effort selection, actual runtime evidence, scoped writes, lifecycle close, and current paid acceptance. Missing evidence deterministically falls back to \`root_inline\`; fallback is not proof of automatic root routing.
...
export function validateActiveActivationRecord(record, { codexHome, policy = null } = {}) {
  ...
  for (const [name, value, expectedPath] of [
    ...
    if (!hasExactRecordFields(value, ["path", "mode", "afterSha256"])) {
