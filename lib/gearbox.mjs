...
export const ACTIVE_ACTIVATION_RECORD_DIRECTORY = "gearbox/activations";
export const ACTIVE_CONFIG_INTEGRITY_SCHEMA_VERSION = 1;
...
- Routing has three separate decisions: task topology selects the Sol root effort, responsibility selects the Luna／Terra／Sol role, and verified runtime capabilities select the execution provider. Workflow-skill policy is a separate gate and cannot retroactively change a stock task's root model or effort after the turn starts.
- Sol Max is selected only for one difficult indivisible task. Sol Ultra requires an explicit packet fact proving at least two independent, disjoint, directly consumable workstreams; child count alone is insufficient.
...
- First active installation requires trusted current ten-question acceptance evidence, a persistent managed activation record, and an applied local rollback manifest. Activation-record and manifest paths are redacted from dispatch status and public evidence; only the managed rollback command may consume the manifest to change global state.
- Active status validates exact Gearbox-owned config blocks plus activation-bound safety semantics; unrelated whole-file config drift is reported but does not turn routing off. Managed-block, unsafe semantic, policy, role, runtime, or permission drift still fails closed with a privacy-safe reason code and component breakdown.
- A legacy activation without a scoped snapshot binds root model and effort to persisted activation smoke and applies the strict safe contract to other semantics; every new apply captures exact scoped semantic equality.
- Policy v2 may enable the separate pre-turn \`app_server_root\` launcher after owner-authorized paid acceptance. \`gearbox-root\` classifies the packet before the first turn, sends the selected Sol model／effort in \`turn/start\`, verifies persisted runtime identity and write scope, reads back the completed turn, archives/unsubscribes, and requires a clean App Server exit. Its acceptance binding and every host file are activation-bound; missing or mismatched evidence returns a reason-coded \`root_inline\` fallback without launching a turn.
- \`app_server_root\` is an explicit foreground launcher, not a Codex core hook, not stock Desktop interception, and not a verified-workflow stage provider. Workflow \`app_thread_root\` remains disabled.
...
export async function readCurrentWorkflowContractEvidence(repositoryRoot) {
  ...
  for (const path of WORKFLOW_CONTRACT_SOURCE_PATHS) {
    ...
    if (
      ...
      await realpath(absolute) !== join(physicalRoot, path)
    ) {
      throw new TypeError(`workflow contract source must be a regular non-symlink repository file: ${path}`);
    }
  ...
  if (
    ...
    await realpath(artifactPath) !==
      join(physicalRoot, "docs", "workflow-contract-evidence.json")
  ) {
    throw new TypeError("workflow contract evidence must be a regular non-symlink repository file");
  }
...
const ACTIVE_CONFIG_MARKERS = Object.freeze([
  CONFIG_LEGACY_THREADS_MARKER,
  CONFIG_ROLES_MARKER,
  CONFIG_V2_MARKER,
]);

function topLevelTomlSource(source) {
  const firstTable = source.search(/^\s*\[/m);
  return firstTable < 0 ? source : source.slice(0, firstTable);
}

function exactTomlString(source, key) {
...
function exactAgentsMaxDepth(source) {
  ...
  const matches = [...body.matchAll(/^max_depth\s*=\s*([^\n#]+)\s*$/gm)];
  if (matches.length !== 1 || !/^\d+$/.test(matches[0][1].trim())) return null;
  return Number(matches[0][1].trim());
}

function activeConfigSemantics(source) {
  const topLevel = topLevelTomlSource(source);
  return {
    model: exactTomlString(topLevel, "model"),
    modelReasoningEffort: exactTomlString(topLevel, "model_reasoning_effort"),
    sandboxMode: exactTomlString(topLevel, "sandbox_mode"),
    approvalPolicy: exactTomlString(topLevel, "approval_policy"),
    agentsMaxDepth: exactAgentsMaxDepth(source),
  };
}

function activeManagedBlockHashes(source) {
  return Object.fromEntries(
    ACTIVE_CONFIG_MARKERS.map((marker) => {
      const block = managedBlock(source, marker);
      return [marker, block === null ? null : sha256(block)];
    }),
  );
}

function exactIntegritySnapshot(value) {
  if (
    value?.schemaVersion !== ACTIVE_CONFIG_INTEGRITY_SCHEMA_VERSION ||
    value?.managedBlockSha256 === null ||
    typeof value?.managedBlockSha256 !== "object" ||
    value?.semanticSettings === null ||
    typeof value?.semanticSettings !== "object"
  ) return false;
  const blockKeys = Object.keys(value.managedBlockSha256).sort();
  const semanticKeys = Object.keys(value.semanticSettings).sort();
  return (
    blockKeys.length === ACTIVE_CONFIG_MARKERS.length &&
    blockKeys.every((key, index) => key === [...ACTIVE_CONFIG_MARKERS].sort()[index]) &&
    Object.values(value.managedBlockSha256).every((digest) => /^[a-f0-9]{64}$/.test(digest)) &&
    semanticKeys.join("\n") === [
      "agentsMaxDepth",
      "approvalPolicy",
      "model",
...
export function validateActiveConfigIntegrity(source, codexHome, expected = null) {
  let managedBlockSha256 = Object.fromEntries(
    ACTIVE_CONFIG_MARKERS.map((marker) => [marker, null]),
  );
  let canonicalBlockSha256 = managedBlockSha256;
  let managedBlocksCanonical = false;
  try {
    managedBlockSha256 = activeManagedBlockHashes(source);
    const canonical = renderConfig(rollbackConfig(source), codexHome, {
      promoteV2: true,
    });
    canonicalBlockSha256 = activeManagedBlockHashes(canonical);
    managedBlocksCanonical = ACTIVE_CONFIG_MARKERS.every(
      (marker) =>
        managedBlockSha256[marker] !== null &&
        managedBlockSha256[marker] === canonicalBlockSha256[marker],
    );
  } catch {
    managedBlocksCanonical = false;
  }

  const semanticSettings = activeConfigSemantics(source);
  const semanticContractSafe = (
    semanticSettings.model === "gpt-5.6-sol" &&
    ACTIVE_ROOT_EFFORTS.includes(semanticSettings.modelReasoningEffort) &&
    ["read-only", "workspace-write"].includes(semanticSettings.sandboxMode) &&
    ["untrusted", "on-failure", "on-request"].includes(
      semanticSettings.approvalPolicy,
    ) &&
    semanticSettings.agentsMaxDepth === 1
  );
  const snapshotValid = expected === null || exactIntegritySnapshot(expected);
  const managedBlocksMatchActivation = expected === null || (
    snapshotValid &&
    ACTIVE_CONFIG_MARKERS.every(
      (marker) => managedBlockSha256[marker] === expected.managedBlockSha256[marker],
    )
  );
  const semanticSettingsMatchActivation = expected === null || (
    snapshotValid &&
    JSON.stringify(semanticSettings) === JSON.stringify(expected.semanticSettings)
  );
  const checks = {
    managedBlocksCanonical,
    managedBlocksMatchActivation,
    semanticContractSafe,
    semanticSettingsMatchActivation,
    snapshotValid,
  };
  let reasonCode = "CONFIG_SCOPED_INTEGRITY_PASS";
  if (!snapshotValid) reasonCode = "CONFIG_INTEGRITY_SNAPSHOT_INVALID";
  else if (!managedBlocksCanonical || !managedBlocksMatchActivation) {
    reasonCode = "CONFIG_MANAGED_BLOCK_DRIFT";
  } else if (!semanticContractSafe || !semanticSettingsMatchActivation) {
    reasonCode = "CONFIG_SAFETY_SEMANTIC_DRIFT";
  }
  return {
    pass: Object.values(checks).every(Boolean),
    reasonCode,
    checks,
    scope: expected === null ? "legacy_safe_contract" : "activation_snapshot",
    managedBlockSha256,
    semanticSettings,
  };
}

export function captureActiveConfigIntegrity(source, codexHome) {
  const validation = validateActiveConfigIntegrity(source, codexHome);
  if (!validation.pass) {
    throw new Error(`Active config integrity preflight failed: ${validation.reasonCode}`);
  }
  return {
    schemaVersion: ACTIVE_CONFIG_INTEGRITY_SCHEMA_VERSION,
    managedBlockSha256: validation.managedBlockSha256,
    semanticSettings: validation.semanticSettings,
  };
}
...
export function captureConfigRollbackState(source) {
  return {
    ...
    managedBlocks: Object.fromEntries(
      ACTIVE_CONFIG_MARKERS.map((marker) => [marker, managedBlock(source, marker)]),
    ),
  };
}
...
const ACTIVE_RECORD_SHA256 = /^[a-f0-9]{64}$/;
const ACTIVE_RECORD_INSTALL_ID = /^[A-Za-z0-9._-]+$/;
const ACTIVE_RECORD_TOP_FIELDS = Object.freeze([
  "schemaVersion",
  "kind",
  "managedBy",
  "status",
...
const ACTIVE_RECORD_ACTIVATION_FIELDS = Object.freeze([
  "installId",
  "recordPath",
  "policySha256",
...
const ACTIVE_RECORD_FILE_FIELDS = Object.freeze({
  role: ["kind", "role", "targetPath", "mode", "afterSha256"],
  launcher: ["kind", "targetPath", "mode", "afterSha256"],
  "dispatch-policy": ["kind", "targetPath", "mode", "afterSha256", "policyMode"],
  "dispatch-runtime": ["kind", "targetPath", "mode", "afterSha256"],
  "dispatch-wrapper": ["kind", "targetPath", "mode", "afterSha256"],
  "dispatch-root-wrapper": ["kind", "targetPath", "mode", "afterSha256"],
});

function plainRecordObject(value) {
  return value !== null && typeof value === "object" && !Array.isArray(value);
}

function hasExactRecordFields(value, fields) {
  return plainRecordObject(value) &&
    JSON.stringify(Object.keys(value).sort()) === JSON.stringify([...fields].sort());
}

export function activeActivationRecordPath(codexHome, installId) {
  if (typeof codexHome !== "string" || resolve(codexHome) !== codexHome) {
    throw new TypeError("CODEX_HOME must be an absolute path");
  }
  if (typeof installId !== "string" || !ACTIVE_RECORD_INSTALL_ID.test(installId)) {
    throw new TypeError("activation installId must be path-safe");
  }
  return join(codexHome, ACTIVE_ACTIVATION_RECORD_DIRECTORY, `${installId}.json`);
}

export function validateActiveActivationRecord(record, { codexHome, policy = null } = {}) {
  const errors = [];
  if (!hasExactRecordFields(record, ACTIVE_RECORD_TOP_FIELDS)) {
    return { pass: false, errors: ["activation record fields are not exact"] };
  }
  if (record.schemaVersion !== 1) errors.push("activation record schemaVersion must equal 1");
  if (record.kind !== "gearbox-active-activation") errors.push("activation record kind is invalid");
  if (record.managedBy !== "sol-ultra-gearbox-v2") errors.push("activation record is unmanaged");
  if (record.status !== "applied") errors.push("activation record is not applied");
  if (!Number.isFinite(Date.parse(record.generatedAt)) || !Number.isFinite(Date.parse(record.completedAt))) {
    errors.push("activation record timestamps are invalid");
  }

  const activation = record.activation;
  if (!hasExactRecordFields(activation, ACTIVE_RECORD_ACTIVATION_FIELDS)) {
    errors.push("activation record binding fields are not exact");
  } else {
    let expectedPath = null;
    try {
      expectedPath = activeActivationRecordPath(codexHome, activation.installId);
    } catch (error) {
      errors.push(error.message);
    }
    if (activation.recordPath !== expectedPath) errors.push("activation record path is not managed");
    for (const field of ACTIVE_RECORD_ACTIVATION_FIELDS.filter((field) => field.endsWith("Sha256"))) {
      if (!ACTIVE_RECORD_SHA256.test(activation[field] ?? "")) {
        errors.push(`activation record ${field} is invalid`);
      }
    }
    if (policy !== null && (
      policy.mode !== "active" ||
      policy.allowTypedBridge !== false ||
      policy.activation?.installId !== activation.installId ||
      policy.activation?.recordPath !== activation.recordPath ||
      policy.sha256 !== activation.policySha256
    )) errors.push("activation record does not match active policy");
  }

  for (const [name, value, expectedPath] of [
    ["config", record.config, join(codexHome, "config.toml")],
    ["agents", record.agents, join(codexHome, "AGENTS.md")],
  ]) {
    const legacyFields = ["path", "mode", "afterSha256"];
    const currentFields = name === "config" ? [...legacyFields, "integrity"] : legacyFields;
    if (!hasExactRecordFields(value, legacyFields) &&
      !hasExactRecordFields(value, currentFields)) {
      errors.push(`activation record ${name} fields are not exact`);
      continue;
    }
    if (value.path !== expectedPath || !Number.isInteger(value.mode) ||
      (value.mode & 0o022) !== 0 ||
      !ACTIVE_RECORD_SHA256.test(value.afterSha256 ?? "")) {
      errors.push(`activation record ${name} binding is invalid`);
    }
    if (name === "config" && Object.hasOwn(value, "integrity") &&
      !exactIntegritySnapshot(value.integrity)) {
      errors.push("activation record config integrity snapshot is invalid");
    }
  }

  if (!hasExactRecordFields(record.staticChecks, ["strictConfig", "configLoad", "mcpConfig", "installation"]) ||
    !Object.values(record.staticChecks ?? {}).every((value) => value === true)) {
    errors.push("activation record static checks are invalid");
  }
  if (!hasExactRecordFields(record.postInstallRootSmoke, ["pass", "actual"]) ||
    record.postInstallRootSmoke.pass !== true ||
    !hasExactRecordFields(record.postInstallRootSmoke.actual, ["persisted", "model", "effort"]) ||
    !validatePostInstallRootRuntime(record.postInstallRootSmoke.actual, { active: true }).pass) {
    errors.push("activation record root smoke is invalid");
  }

  if (!Array.isArray(record.files) || record.files.length === 0) {
    errors.push("activation record files are invalid");
  } else {
    const identities = new Set();
    for (const entry of record.files) {
      const fields = ACTIVE_RECORD_FILE_FIELDS[entry?.kind];
      if (!fields || !hasExactRecordFields(entry, fields)) {
        errors.push("activation record file fields are invalid");
        continue;
      }
      if (!isAbsolute(entry.targetPath) || !Number.isInteger(entry.mode) ||
        (entry.mode & 0o022) !== 0 || !ACTIVE_RECORD_SHA256.test(entry.afterSha256 ?? "")) {
        errors.push("activation record file binding is invalid");
      }
      if (entry.kind === "role" && (typeof entry.role !== "string" || entry.role.length === 0)) {
        errors.push("activation record role is invalid");
      }
      if (entry.kind === "dispatch-policy" && entry.policyMode !== "active") {
        errors.push("activation record policy mode is invalid");
      }
      const identity = `${entry.kind}:${entry.role ?? ""}:${entry.targetPath}`;
      if (identities.has(identity)) errors.push("activation record file is duplicated");
...
export function createActiveActivationRecord({ manifest, codexHome, policy }) {
  if (manifest?.status !== "applied" || !Array.isArray(manifest.files)) {
    throw new TypeError("applied install manifest is required for activation record");
  }
  const files = manifest.files.map((entry) => {
    const mode = entry.mode ?? (entry.kind === "role" ? 0o644 : null);
    const base = {
      kind: entry.kind,
      ...(entry.kind === "role" ? { role: entry.role } : {}),
      targetPath: entry.targetPath,
      mode,
      afterSha256: entry.afterSha256,
      ...(entry.kind === "dispatch-policy" ? { policyMode: entry.policyMode } : {}),
    };
    if (!Object.hasOwn(ACTIVE_RECORD_FILE_FIELDS, entry.kind)) {
      throw new TypeError(`unsupported activation record file kind: ${entry.kind}`);
    }
    return base;
  });
  const record = {
    schemaVersion: 1,
    kind: "gearbox-active-activation",
    ...
    activation: {
      ...
      recordPath: manifest.activation?.recordPath,
      policySha256: manifest.activation?.policySha256,
      acceptanceBindingSha256: manifest.activation?.acceptanceBindingSha256,
      writingSkillsEvidenceSha256: manifest.activation?.writingSkillsEvidenceSha256,
      workflowContractEvidenceSha256: manifest.activation?.workflowContractEvidenceSha256,
    },
    config: {
      path: manifest.config?.path,
      mode: manifest.config?.mode,
      afterSha256: manifest.config?.afterSha256,
      ...(manifest.config?.integrity === undefined
        ? {}
        : { integrity: manifest.config.integrity }),
    },
    agents: {
      path: manifest.agents?.path,
      mode: manifest.agents?.mode,
      afterSha256: manifest.agents?.afterSha256,
    },
    staticChecks: {
      strictConfig: manifest.staticChecks?.strictConfig,
      configLoad: manifest.staticChecks?.configLoad,
      mcpConfig: manifest.staticChecks?.mcpConfig,
      installation: manifest.staticChecks?.installation,
    },
    postInstallRootSmoke: {
      pass: manifest.postInstallRootSmoke?.pass,
      actual: {
        persisted: manifest.postInstallRootSmoke?.actual?.persisted,
  ...
  const validation = validateActiveActivationRecord(record, { codexHome, policy });
  if (!validation.pass) {
    throw new TypeError(`invalid activation record: ${validation.errors.join("; ")}`);
  }
  return record;
}

export async function persistActiveActivationRecord({ codexHome, manifest, policy }) {
  const record = createActiveActivationRecord({ manifest, codexHome, policy });
  const path = record.activation.recordPath;
  const directory = dirname(path);
  await mkdir(directory, { recursive: true, mode: 0o700 });
  const directoryMetadata = await lstat(directory);
  if (!directoryMetadata.isDirectory() || directoryMetadata.isSymbolicLink()) {
    throw new TypeError("activation record directory must be a real directory");
  }
  await chmod(directory, 0o700);
  const physicalCodexHome = await realpath(codexHome);
  if (await realpath(directory) !== join(physicalCodexHome, ACTIVE_ACTIVATION_RECORD_DIRECTORY)) {
    throw new TypeError("activation record directory must remain under physical CODEX_HOME");
  }
  const source = `${JSON.stringify(redactSensitive(record), null, 2)}\n`;
  const existing = await readOptional(path);
  if (existing !== null && existing !== source) {
    throw new TypeError("activation record target already contains different content");
  }
  if (existing === null) await atomicWrite(path, source, 0o600);
  const metadata = await lstat(path);
  const installed = await readFile(path, "utf8");
  if (!metadata.isFile() || metadata.isSymbolicLink() || (metadata.mode & 0o777) !== 0o600 || installed !== source) {
    throw new Error("activation record readback failed");
  }
  return { path, sha256: sha256(installed), record };
}

export async function verifyActiveActivationRecordForRemoval({
  codexHome,
  activation,
  expectedSha256 = null,
}) {
  const path = activeActivationRecordPath(codexHome, activation?.installId);
  if (activation?.recordPath !== path) {
    throw new TypeError("activation record removal path is not managed");
  }
  let metadata;
  try {
    metadata = await lstat(path);
  } catch (error) {
    if (error?.code === "ENOENT") return { path, exists: false, sha256: null };
    throw error;
  }
  if (!metadata.isFile() || metadata.isSymbolicLink() || (metadata.mode & 0o777) !== 0o600) {
    throw new TypeError("activation record removal requires a private regular file");
  }
  const physicalCodexHome = await realpath(codexHome);
  if (await realpath(path) !== join(
    physicalCodexHome,
    ACTIVE_ACTIVATION_RECORD_DIRECTORY,
    `${activation.installId}.json`,
  )) throw new TypeError("activation record removal path escaped CODEX_HOME");
  const source = await readFile(path, "utf8");
  const digest = sha256(source);
  if (expectedSha256 !== null && (
    !ACTIVE_RECORD_SHA256.test(expectedSha256) || digest !== expectedSha256
  )) throw new Error("activation record changed after installation");
  let record;
  try {
    record = JSON.parse(source);
  } catch {
    throw new TypeError("activation record removal requires managed JSON");
  }
  const validation = validateActiveActivationRecord(record, { codexHome });
  if (!validation.pass || record.activation.installId !== activation.installId ||
    record.activation.recordPath !== activation.recordPath ||
    (activation.policySha256 !== undefined &&
      record.activation.policySha256 !== activation.policySha256)) {
    throw new TypeError("activation record removal binding is invalid");
  }
  return { path, exists: true, sha256: digest };
}

export async function removeActiveActivationRecord({
  codexHome,
  activation,
  expectedSha256 = null,
}) {
  const verified = await verifyActiveActivationRecordForRemoval({
    codexHome,
    activation,
    expectedSha256,
  });
  if (!verified.exists) return { path: verified.path, action: "already_absent" };
  await unlink(verified.path);
  return { path: verified.path, action: "removed" };
}
...
function dispatchRuntimeEntries({ sourceRoot, codexHome, policySource }) {
  return [
    ...
    },
    {
      ...
      sourcePath: join(sourceRoot, "scripts", "gearbox-root"),
      targetPath: join(codexHome, "bin", "gearbox-root"),
      mode: 0o755,
    },
  ];
}
