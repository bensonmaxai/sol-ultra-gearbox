...
export async function installDispatchRuntime({ sourceRoot, codexHome, backupDirectory, dispatchMode }) {
  if (dispatchMode === "active") {
    throw new Error("active dispatch requires trusted acceptance evidence");
  }
  if (dispatchMode !== "shadow") throw new Error("dispatch mode must be shadow");
  ...
  for (const entry of dispatchRuntimeEntries({ sourceRoot, codexHome, policySource })) {
    ...
    await atomicWrite(entry.targetPath, source, entry.mode);
    const installed = await regularFileOrAbsent(entry.targetPath);
    const targetSource = await readFile(entry.targetPath, "utf8");
    const sourceSha256 = sha256(source);
    const targetSha256 = sha256(targetSource);
