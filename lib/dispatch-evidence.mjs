...
const HARD_FAILURE_CHECKS = Object.freeze([
  "runtimePersisted",
  "modelMatches",
  "effortMatches",
  "sandboxMatches",
...
function sessionId(summary) {
  return summary?.sessionId ?? summary?.sessionMeta?.id ?? summary?.sessionMeta?.session_id ?? null;
}

function parentThreadId(summary) {
  return summary?.sessionMeta?.source?.subagent?.thread_spawn?.parent_thread_id ?? null;
}
...
function filesystemScope({ roleSpec, files, allowedWriteScope }) {
  if (!Array.isArray(files) || !files.every(scopeEntry)) return false;
  if (roleSpec?.sandbox === "read-only") return files.length === 0;
  if (roleSpec?.sandbox !== "workspace-write") return false;
  return (
    Array.isArray(allowedWriteScope) &&
    allowedWriteScope.length > 0 &&
    allowedWriteScope.every(scopeEntry) &&
    files.every((path) => allowedWriteScope.some((scope) => isWithinScope(path, scope)))
  );
}

function rollbackRequired(checks) {
  return HARD_FAILURE_CHECKS.some((name) => checks[name] !== true);
}
...
function resultEnvelope({ decision, roleSpec, roleHash, actual, checks, changedFiles: files, retryCount = 0, synthetic = false }) {
  ...
  return {
    ...
    changedFiles: files ?? [],
    retryCount,
    rollbackRequired: rollbackRequired(checks),
    synthetic,
  };
}
  ...
  const checks = {
    ...
    tokenShape:
      (decision?.selectedShape === "isolated_role_root" &&
        isNonNegativeFinite(result?.actual?.parentTokens) &&
        result?.actual?.childTokens === null) ||
      (decision?.selectedShape === "typed_child" &&
        isNonNegativeFinite(result?.actual?.parentTokens) &&
        isNonNegativeFinite(result?.actual?.childTokens)),
...
export function verifyIsolatedRoot({ summary, decision, roleSpec, roleHash, before, after, cleanup }) {
  ...
  const checks = {
    ...
    filesystemScope:
      Array.isArray(changedFiles(before)) &&
      filesystemScope({ roleSpec, files, allowedWriteScope: [] }),
    ...cleanupChecks,
    deliverableValid:
      decision?.selectedShape === "isolated_role_root" &&
...
export function verifyTypedChildResult({
  parent,
  child,
  decision,
  roleSpec,
  ...
  const checks = {
    runtimePersisted: Boolean(
      parent?.sessionMeta &&
        ...
        typeof sessionId(parent) === "string" &&
        sessionId(parent).length > 0 &&
        parentThreadId(child) === sessionId(parent),
    ),
    modelMatches: actual.model === roleSpec?.model,
    effortMatches: actual.effort === roleSpec?.effort,
    sandboxMatches: actual.sandbox === roleSpec?.sandbox,
...
export function classifyDispatchFailure(result) {
  ...
  const hardFailure = HARD_FAILURE_CHECKS.filter(
    (name) => name !== "filesystemScope",
