...
export function validateWorkflowEvent(event) {
  ...
  switch (event.type) {
    ...
    case "stage_ready":
      ...
      if (!isSafeId(event.stageId) || (Object.hasOwn(event, "resolvedFact") && !isSafeId(event.resolvedFact))) errors.push("stage_ready is invalid");
      break;
    case "batch_planned":
    ...
    case "materialized":
      ...
      if (Object.hasOwn(event, "dispatchResult") && (!event.dispatchResult || typeof event.dispatchResult !== "object" || Array.isArray(event.dispatchResult) || event.status !== "completed")) errors.push("isolated receipt is invalid");
    ...
    case "provider_closed":
      ...
      if (!isSafeId(event.stageId) || !["adopted", "rejected", "blocked"].includes(event.disposition) || typeof event.cleanupPassed !== "boolean") errors.push("provider_closed is invalid");
      break;
...
function materializationHash(planHash, stageId, attempt, event) {
  if (event.materializationHash) {
    requireValid(attempt.executionShape !== "isolated_role_root" || event.status === "completed", "completed isolated receipt is required");
    return event.materializationHash;
  }
...
export function reduceWorkflowEvent({ plan, state, event }) {
  ...
  switch (event.type) {
    ...
    case "rejected": {
      ...
      const attempt = currentAttempt(stage);
      attempt.rejection = { final: event.final, hardFailure: event.hardFailure, reasonCode: event.reasonCode };
    ...
    case "provider_closed": {
      ...
      requireValid(
        (event.disposition === "adopted" && stage.state === "adopted")
        || (event.disposition === "rejected" && stage.state === "rejected")
        || (event.disposition === "blocked" && stage.state === "blocked"),
        "provider disposition does not match stage",
      );
      const attempt = currentAttempt(stage);
      ...
      if (stage.state === "adopted" || (stage.state === "rejected" && stage.finalRejection)) stage.state = "closed";
      clearCompletedBatch(next);
      break;
    }
    case "correction_authorized": {
    ...
    case "stage_blocked": {
      ...
      const deferredStageIds = next.activeBatch?.canaryStageId === event.stageId
        ? next.activeBatch.stageIds.slice(1)
        : [];
      const deferredMaterialized = deferredStageIds.some((deferredId) => next.stages[deferredId].attempts.some(
        (attempt) => isHash(attempt.materializationHash),
      ));
      if (deferredStageIds.length > 0 && !deferredMaterialized) {
        for (const deferredId of deferredStageIds) {
          const deferred = next.stages[deferredId];
          if (!deferred.attempts.some((attempt) => isHash(attempt.materializationHash)) && deferred.state !== "closed") {
