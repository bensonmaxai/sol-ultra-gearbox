...
export const OBSERVED_USAGE_REPORT_BASENAME = "real-work-usage.json";
...
const ROOT_PROVIDER_FIELDS = Object.freeze([
  "status",
  "provider",
  "model",
  "effort",
...
export function validateRootProviderSummary(value) {
  const exactFields = value !== null && typeof value === "object" && !Array.isArray(value) &&
    JSON.stringify(Object.keys(value).sort()) === JSON.stringify([...ROOT_PROVIDER_FIELDS].sort());
  const checks = {
    exactFields,
    status:
      value?.status === "pass" &&
      value?.provider === "app_server_root" &&
      value?.reasonCode === "APP_SERVER_ROOT_RUNTIME_VERIFIED",
    route:
      value?.model === "gpt-5.6-sol" &&
      value?.effort === "low",
    hashes:
      [value?.taskHash, value?.receiptSha256, value?.policySha256]
        .every((entry) => SHA256.test(entry ?? "")),
    evidence:
      value?.scopeVerified === true &&
      value?.smokeTaskBound === true &&
      value?.rolloutReverified === true &&
      value?.lifecycleClosed === true &&
      value?.persistedRuntime === true &&
      value?.tokenUsagePersisted === true,
  };
  return { pass: Object.values(checks).every(Boolean), checks };
}

export function validateWorkflowContractSummary(value) {
  const exactFields = value !== null && typeof value === "object" && !Array.isArray(value) &&
    Object.keys(value).length === WORKFLOW_CONTRACT_FIELDS.length &&
    WORKFLOW_CONTRACT_FIELDS.every((field) => Object.hasOwn(value, field));
  const checks = {
    exactFields,
    deterministicScenarios:
      value?.deterministicScenarioCount === 5 && value?.deterministicPass === true,
    q10Canary: value?.q10CanaryVerified === true,
  };
  return { pass: Object.values(checks).every(Boolean), checks };
}
...
- Verified workflow contract: ${workflow.deterministicPass === true ? "PASS" : "FAIL"} (${workflow.deterministicScenarioCount ?? 0}/5); Q10 canary: ${workflow.q10CanaryVerified === true ? "verified" : "unverified"}
...
- Writing-skills pressure test: ${writingSkills.pass === true ? "PASS" : "NOT VERIFIED"} (${writingSkills.redRuns ?? 0} RED, ${writingSkills.greenRuns ?? 0} GREEN), isolated role \`${writingSkills.role?.name ?? "unavailable"}\`
...
- App Server root launcher: ${rootProvider.status === "pass" ? "PASS" : "NOT VERIFIED"}; \`${rootProvider.model ?? "unavailable"}\` / ${rootProvider.effort ?? "unavailable"}; fixed smoke ${rootProvider.smokeTaskBound === true ? "bound" : "unverified"}; persisted rollout ${rootProvider.rolloutReverified === true ? "reverified" : "unverified"}; scope ${rootProvider.scopeVerified === true ? "verified" : "unverified"}; lifecycle ${rootProvider.lifecycleClosed === true ? "closed" : "unverified"}
${roleTable}
...
export function validateReleaseEvidence({ evidence, markdown, currentSource }) {
  ...
  const rootProvider = validateRootProviderSummary(evidence?.runtime?.rootProvider);
  const workflowContract = validateWorkflowContractSummary(
    evidence?.workflowContract,
  );
  const completePairCount = cost.completePairCount;
  const requiredPairCount = cost.requiredPairCount;
  const writingSkills = evidence?.runtime?.roleSmoke?.writingSkillsAdapter;
  ...
  const checks = {
    ...
    writingSkillsAdapterPassed:
      writingSkills?.pass === true &&
      writingSkills?.role?.name === "sol_skill_tester" &&
      writingSkills?.role?.model === "gpt-5.6-sol" &&
      writingSkills?.role?.effort === "high" &&
      writingSkills?.role?.sandbox === "read-only" &&
      Number.isInteger(writingSkills?.redRuns) &&
      writingSkills.redRuns >= 5 &&
      Number.isInteger(writingSkills?.greenRuns) &&
      writingSkills.greenRuns >= 5 &&
      SHA256.test(writingSkills?.evidenceSha256 ?? ""),
    sddAdapterPassed:
    ...
    rootProvider: rootProvider.pass,
    workflowContract: workflowContract.pass,
    costBoundary:
      cost.kind === "real_work" &&
      Number.isInteger(completePairCount) &&
      completePairCount >= 0 &&
