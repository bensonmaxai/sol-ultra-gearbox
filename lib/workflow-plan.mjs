...
function graphErrors(plan, indexes, errors) {
  ...
  const outputArtifacts = new Set(inputArtifacts);
  const visiting = new Set();
  ...
  for (const stage of plan.stages) {
    ...
    const stageInputs = Array.isArray(stage.inputArtifacts) ? stage.inputArtifacts : [];
    for (const artifact of stageInputs) {
