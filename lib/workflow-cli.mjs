...
const UNSAFE_WORKFLOW_STRINGS = Object.freeze([
  /\/(?:Users|home)\/(?!example(?:\/|$)|test(?:\/|$)|username(?:\/|$))[^\s"'`]*/i,
  /\/(?:private|tmp)\//i,
...
export function workflowOutputIsUnsafe(value, seen = new WeakSet()) {
  if (typeof value === "string") return UNSAFE_WORKFLOW_STRINGS.some((pattern) => pattern.test(value));
  if (!value || typeof value !== "object") return false;
  if (seen.has(value)) return true;
  seen.add(value);
  const unsafe = Array.isArray(value)
    ? value.some((item) => workflowOutputIsUnsafe(item, seen))
    : Object.values(value).some((item) => workflowOutputIsUnsafe(item, seen));
  seen.delete(value);
  return unsafe;
}

function renderedStageValues(plan) {
...
function assertExpectedMaterialization({ plan, planHash, state, policy, capabilities, roleSpecs, event }) {
  ...
  let stageIds = batch.stageIds;
  if (batch.stageIds.length === 1) {
    const candidate = selectCandidateBatch({ plan, state });
    if (!Object.hasOwn(candidate, "kind") && candidate.stageIds.includes(event.stageId)) {
...
export async function runWorkflowNext({ envelope, policy, capabilities, roleSpecs = ROLE_SPECS, cwd }) {
  ...
  const sourceCompatible = compatibleStateSource(envelope?.stateSource);
  if (!sourceCompatible && stateSource === "upstream") {
    return blocked({ policy, stateSource, reasonCode: "WORKFLOW_UPSTREAM_STORE_INCOMPATIBLE" });
  }
  if (!envelopeValidation.pass) {
    return blocked({ policy, stateSource, reasonCode: "WORKFLOW_ENVELOPE_INVALID" });
  }
  ...
  if (workflowOutputIsUnsafe(renderedStageValues(envelope.plan))) {
    return blocked({ policy, stateSource, reasonCode: "WORKFLOW_OUTPUT_UNSAFE" });
  }
  let binding;
  try {
  ...
  try {
    if (persisted.length === 0) {
      ...
      if (!resumed.pass && resumed.reasonCode !== "WORKFLOW_INCOMPLETE_EXECUTION") {
        const reasonCode = stateSource === "upstream" && resumed.reasonCode === "WORKFLOW_LEDGER_INVALID"
          ? "WORKFLOW_UPSTREAM_STORE_INCOMPATIBLE"
          : resumed.reasonCode;
        return blocked({ policy, stateSource, reasonCode });
      }
