...
const SUPPORTED_APP_SERVER_VERSIONS = new Set(["0.144.2", "0.145.0"]);

export const APP_SERVER_ROOT_SMOKE_MARKER = "GEARBOX_APP_SERVER_ROOT_OK";
export const APP_SERVER_ROOT_HISTORY_MODE = "legacy";
...
class AppServerRequestError extends Error {
  constructor(method, error) {
    super(`App Server request rejected: ${method}`);
    this.name = "AppServerRequestError";
    this.method = method;
    this.code = Number.isInteger(error?.code) ? error.code : null;
    this.serverMessageSha256 = sha256(String(error?.message ?? ""));
  }
}
...
class JsonRpcStdioClient {
  ...
  #onLine(line) {
    ...
    if (Object.hasOwn(value, "id") && !Object.hasOwn(value, "method")) {
      ...
      if (value.error) pending.reject(new AppServerRequestError(pending.method, value.error));
...
function failureCode(error) {
  if (error instanceof AppServerRequestError) {
    return {
      initialize: "APP_SERVER_ROOT_INITIALIZE_REJECTED",
      "thread/start": "APP_SERVER_ROOT_THREAD_START_REJECTED",
      "turn/start": "APP_SERVER_ROOT_TURN_START_REJECTED",
      "thread/read": "APP_SERVER_ROOT_READBACK_REJECTED",
      "thread/archive": "APP_SERVER_ROOT_ARCHIVE_REJECTED",
      "thread/unsubscribe": "APP_SERVER_ROOT_UNSUBSCRIBE_REJECTED",
      "turn/interrupt": "APP_SERVER_ROOT_INTERRUPT_REJECTED",
    }[error.method] ?? "APP_SERVER_ROOT_REQUEST_REJECTED";
  }
  const message = error instanceof Error ? error.message : "";
  if (message.includes("thread/start identity")) {
    return "APP_SERVER_ROOT_THREAD_START_INVALID_RESPONSE";
  }
  if (message.includes("turn/start identity")) {
    return "APP_SERVER_ROOT_TURN_START_INVALID_RESPONSE";
  }
...
export async function runAppServerRoot({
  ...
  try {
    ...
    const threadStart = await client.request("thread/start", {
      ...
      ephemeral: false,
      historyMode: APP_SERVER_ROOT_HISTORY_MODE,
    });
    ...
    failureStage = "turn_started_notification";
    ...
    let completed;
    try {
      failureStage = "turn_completed_notification";
    ...
    const receipt = {
      ...
      lifecycle: {
        initialized: initializeCompleted,
      ...
      diagnostics: {
        ...
        stderrSha256: sha256(client.stderr),
        failureStage,
        serverErrorCode: error instanceof AppServerRequestError ? error.code : null,
        serverErrorMessageSha256:
          error instanceof AppServerRequestError ? error.serverMessageSha256 : null,
      },
    };
    const persisted = await persistReceipt(targetReceiptPath, receipt);
    return {
