...
function isDefaultLedgerPath(path) {
  return resolve(path) === resolve(DEFAULT_DISPATCH_LEDGER_PATH);
}

function ensureLedgerParent(path) {
  ...
  if ((parentStat.mode & 0o777) !== 0o700) {
    if (!isDefaultLedgerPath(path)) {
      throw new TypeError("dispatch ledger parent must be an owned 0700 directory");
    }
    chmodSync(parent, 0o700);
  }
}

function assertCompleteLedger(path) {
  ...
  try {
    ...
    readSync(descriptor, lastByte, 0, 1, fileStat.size - 1);
    if (lastByte[0] !== 0x0a) {
...
export function appendDispatchRecord(path = DEFAULT_DISPATCH_LEDGER_PATH, record) {
  ...
  try {
    ...
    if (writeSync(descriptor, line, null, "utf8") !== byteLength) {
      throw new TypeError("dispatch ledger append was incomplete");
    }
